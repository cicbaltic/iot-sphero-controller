...
class IotDevice extends EventEmitter {
	...
	connect() {
		...
		this.device
			.on('connect', (err, data) => {
				this.logger.debug('Connected to IOT Foundation', this.config.id);
				this.emit('connected', this.config.device, data);
			})
			.on('command', (command, format, payload, topic) => {
				this.logger.debug(`Command received`, command, format, topic, JSON.parse(payload));
