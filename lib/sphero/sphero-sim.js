...
class SpheroSimDevice extends EventEmitter {
	constructor(config) {
		...
		this.streamingAccelerometer = false;
		this.streamingOdometer = false;
	}
	...
	startCollisionDetection() {
		if (!this.isConnected()) return;
		if (this.detectingCollisions) return;
	...
	streamAccelerometer(accel, sps) {
		...
		if (this.streamingAccelerometer && accel) return;

		if (!accel) {
			clearInterval(this.accelerometer);
			return;
		}

		this.streamingAccelerometer = true;

		this.accelerometer = setInterval(() => {
	...
	streamOdometer(odor, sps) {
		...
		if (this.streamingOdometer && odor) return;

		if (!odor) {
			clearInterval(this.odormeter);
			return;
		}

		this.streamingOdometer = true;

		this.odormeter = setInterval(() => {
			this.emit('odometer', {
				Odometer: {
					sensor: 'odomoter X',
					range: {
